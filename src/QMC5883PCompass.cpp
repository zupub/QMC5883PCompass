...
===============================================================================================================
QMC5883PCompass.h
Library for using QMC5583P series chip boards as a compass.
Base on [https://github.com/mprograms/QMC5883LCompass]
...
Over sample Ratio (OSR1)
    8         	    0x00
    4         	    0x10
    2         	    0x20
    1             	0x30

Down sample Ratio (OSR2)
    1         	    0x00
    2         	    0x40
    4         	    0x80
    8          	    0xC0
...
{
    Wire.begin();
    setMode(0x01, 0x0C, 0x10, 0X00, 0x08);
}
...
void QMC5883PCompass::setADDR(byte b)
{
    _ADDR = b;
}
...
**/
// Set chip mode
void QMC5883PCompass::setMode(byte mode, byte odr, byte osr1, byte osr2, byte rng)
{
    _writeReg(0x0A, mode | odr | osr1 | osr2);
    _writeReg(0x0B, rng);
}
...
**/
void QMC5883PCompass::setCalibration(int x_min, int x_max, int y_min, int y_max, int z_min, int z_max)
{
